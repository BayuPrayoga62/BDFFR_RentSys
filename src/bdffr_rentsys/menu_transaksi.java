...
public class menu_transaksi extends javax.swing.JFrame {
    ...
    try {
        String sql = "SELECT " +
                     "costumer.nama AS Peminjam, " +
                     "mobil.nomor_kendaraan AS `No. Polisi`, " +
                     "transaksi.created_at AS `Tgl. Pinjam`, " +
                     "transaksi.updated_at AS `Tgl. Kembali`, " +
                     "transaksi.lama_hari_sewa AS `Lama Pinjam`, " +
                     "transaksi.total_harga_sewa AS Total " +
                     "FROM transaksi " +
        ...
        model.setColumnIdentifiers(new String[] {
            "Peminjam", "No. Polisi", "Tgl. Pinjam", "Tgl. Kembali", "Lama Pinjam", "Total"
        });
        ...
        while (rs.next()) {
            model.addRow(new Object[] {
                ...
                rs.getString("Tgl. Pinjam"),
                rs.getString("Tgl. Kembali"),
                rs.getInt("Lama Pinjam"),
                rs.getDouble("Total")
            });
        }
...
//                     "JOIN mobil ON transaksi.id_mobil = mobil.id";
//        
//        rs = con.queryCustom(sql); // Pastikan method queryCustom di koneksidb Anda benar
...
//    }
    ...
    public void add_peminjam() {
        String kolom[] = {"nama", "jenis_kelamin", "alamat", "no_hp", "email"};
        String isi[]   = {ShowNama.getText(),ShowNIK.getText(), ShowAlamat.getText(), 
                          ShowNoTelp.getText(), ShowEmail.getText()};
        System.out.println(con.queryInsert("costumer", kolom, isi));
   ...
   public String getIdCustomerByEmail(String email) {
    try {
        ResultSet rs = con.querySelectAll("costumer", "email='" + email + "'");
        if (rs.next()) {
    ...
    public String getIdMobilByNopol(String nopol) {
        try {
            ResultSet rs = con.querySelectAll("mobil", "nomor_kendaraan='" + nopol + "'");
            if (rs.next()) {
                return rs.getString("id");
            }
        } catch (SQLException e) {
    ...
    try {
        if (!ShowNama.getText().isEmpty()) {
            ...
            // Cek dan tambahkan costumer jika belum ada
            if (!isCustomerExist(ShowEmail.getText())) {
                add_peminjam();  // ← INSERT customer
            }

            // Ambil ID setelah data costumer pasti sudah masuk
            String id_customer = getIdCustomerByEmail(ShowEmail.getText());
            String id_mobil = getIdMobilByNopol(cb_nopol.getSelectedItem().toString());

            if (id_customer == null || id_mobil == null) {
            System.out.println("DEBUG: Email: " + ShowEmail.getText() + ", Nopol: " + cb_nopol.getSelectedItem().toString());
            JOptionPane.showMessageDialog(this, "Gagal mendapatkan ID Costumer atau ID Mobil");
            return;
        }


            java.util.Date tglPinjam = DateChooser1.getDate();
            java.util.Date tglKembali = DateChooser2.getDate();

            String kolom[] = {
                "id_costumer", "id_mobil",
                "jumlah_sewa", "lama_hari_sewa", "total_harga_sewa",
                "jenis_transaksi", "created_at", "updated_at"
            };
            ...
            String isi[] = {
                ...
                ShowTotal.getText(),
                "dipinjam",
                new java.sql.Date(tglPinjam.getTime()).toString(),
                new java.sql.Date(tglKembali.getTime()).toString()
            };

            String result = con.queryInsert("transaksi", kolom, isi);
            ...
            if (result != null && !result.toLowerCase().contains("error")) {
                ...
                JOptionPane.showMessageDialog(this, "Gagal menyimpan data transaksi: " + result);
            }
...
//                    JOptionPane.showMessageDialog(this, "Maaf Mobil Ini Sedang Tidak Tersedia");
//                } else {
//                    String kolom[] = {"id", "id_costumer", "id_mobil", "nomor_kendaraan", "jumlah_sewa", "lama_hari_sewa", "total_harga_sewa", "jenis_transaksi", "created_at", "updated_at"};
...
//    }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("BDFFR RentSys | Sewa Mobil");
            ...
            .addGroup(jPanel3Layout.createSequentialGroup()
                ...
                .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel3Layout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                        .addComponent(ShowNIK, javax.swing.GroupLayout.PREFERRED_SIZE, 119, javax.swing.GroupLayout.PREFERRED_SIZE))
                    .addGroup(jPanel3Layout.createSequentialGroup()
                        .addGap(1, 1, 1)
                        .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
...
//      add_peminjam();
    ...
    private void cb_nopolActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cb_nopolActionPerformed
        ...
        ResultSet rst = con.querySelectAll("mobil", "nomor_kendaraan='" + st + "'");
        try {
            while (rst.next()) {

                this.ShowMerk.setText(rst.getString("nama_mobil"));
                this.ShowTipe.setText(rst.getString("jenis"));
